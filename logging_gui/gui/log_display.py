...
class LogDisplay(tk.Frame):
    ...
    def _load_custom_logs(self):
        try:
            custom_log_file = Path(DATA_DIR) / "custom_logs.json"
    ...
    def create_widgets(self):
        ...
        # --- Navigation controls (including Case and Regex buttons) ---
    ...
    def _perform_search_and_filter_logs(self, scroll_to_end=False):
        ...
        for i, log_entry in enumerate(self.all_logs):
            ...
            if log_entry.get('state') != 'DELETED' and level_should_display:
                ...
                # Insert text piece by piece to handle links correctly
                if not links:
                ...
                # Perform search on the newly inserted line
                if search_term:
        ...
        # Add all found matches to the main search highlight tag
        ...
        # Update search count and highlight first match
    ...
    def _handle_log_area_click(self, event):
        ...
        if self.selected_log_line_index == line_start or not self.log_area.get(line_start, f"{line_start} lineend").strip():
            ...
            if self.editor_window_instance and self.editor_window_instance.winfo_exists():
                self.editor_window_instance.update_content(None) # Reset editor to 'Add' mode on deselection
        else:
    ...
    def _on_comment_entry_focus_out(self, event):
        ...
        if self.autocomplete_popup and self.autocomplete_popup.winfo_exists():
            if new_focus is not self.autocomplete_popup and new_focus not in self.autocomplete_popup.winfo_children():
        ...
        if not self.comment_entry.get(1.0, tk.END).strip():
            is_idle = not self.is_running and not self.log_file_open
            if is_idle:
                self.comment_entry.delete(1.0, tk.END)
                self.comment_entry.insert(tk.END, "Idle: Run or open a log file to start.")
                self.comment_entry.config(fg=self.theme.DISABLED_TEXT_COLOR)
    ...
    def add_log(self, message, level='INFO', to_file=True, scroll=True):
        # This is a simplified version for real-time logs. 
        # A full search/filter is not performed for every single new log for performance reasons.
    ...
    def toggle_run_exit(self):
        """
        # 스크립트 실행 또는 종료를 토글합니다.
        # Toggles between running and exiting the script.
        """
        if self.is_running:
            ...
            # 1. 입력에 공백이 포함된 경우, 사용자가 인터프리터를 직접 지정했다고 가정합니다.
            if ' ' in command_input:
                command_to_run = shlex.split(command_input)
            else:
                # 2. 파일 경로만 입력된 경우, 먼저 파일이 실제 존재하는지 확인합니다.
                ...
                # 확장자를 기반으로 인터프리터 맵을 확인합니다.
                ext = script_path.suffix.lower()
                interpreter_found = False

                if ext in INTERPRETER_MAP:
                    interpreter_commands = INTERPRETER_MAP[ext]
                    for cmd in interpreter_commands:
                        if shutil.which(cmd):
                            command_to_run = [cmd, str(script_path)]
                            self.update_status(f"Found interpreter '{cmd}' for '{ext}' file.", self.theme.ACCENT_COLOR)
                            interpreter_found = True
                            break
                    
                # 3. 맵에서 인터프리터를 찾지 못했거나 파일에 실행 권한이 있다면, 직접 실행을 시도합니다.
                if not interpreter_found:
                    # 파일에 실행 권한이 있는지 확인
                    if os.access(script_path, os.X_OK):
                        command_to_run = [str(script_path)]
                        self.update_status("인터프리터가 지정되지 않았습니다. 파일 자체를 직접 실행합니다.", self.theme.WARNING_COLOR)
                    else:
    ...
    def process_log_queue(self):
        ...
        try:
            while True:
                ...
                log_level_pattern = rf"\s*(\[({all_log_types})])"
                level_match = re.search(log_level_pattern, msg_text, re.I)
                level = level_match.group(2).upper() if level_match else 'INFO'
                if level == 'PROGRESS':
    ...
    def _load_log_file(self, file_path: str):
        ...
        try:
            self.all_logs, self.undo_stack, self.redo_stack = [], [], []
            all_log_types = "|".join(re.escape(k) for k in self.theme.LOG_LEVEL_COLORS.keys())
            log_pattern = re.compile(rf"\s*(\[({all_log_types})])", re.I)
            ...
            for line in processed_lines:
                ...
                level = match.group(2).upper() if match else "INFO"
                if level == 'PROGRESS':
                    level = 'INFO'
...
# [추가됨] 이 파일을 직접 실행할 수 있도록 하는 코드
if __name__ == "__main__":
    # tkinterdnd2 지원 여부에 따라 루트 윈도우 생성
    if DND_SUPPORT:
        root = TkinterDnD.Tk()
    else:
        root = tk.Tk()
    
    # config.py에서 기본 스크립트 경로와 로그 디렉토리 경로 가져오기
    # SCRIPT_PATH가 실제 파일일 경우에만 초기값으로 설정
    initial_script_path = SCRIPT_PATH if Path(SCRIPT_PATH).is_file() else ""
    initial_log_dir = LOG_DIR
    
    # LogDisplay 애플리케이션 인스턴스 생성 및 실행
